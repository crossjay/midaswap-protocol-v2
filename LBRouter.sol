...
contract LBRouter is ILBRouter {
    ...
    /// @dev Receive function that only accept ETH from the WETH contract
    receive() external payable {
        // if (msg.sender != address(weth)) revert LBRouter__SenderIsNotWETH(); 
    }
    ...
    function getIdFromPrice(uint256 _price) external view override returns (uint24) {
        return BinHelper.getIdFromPrice(_price);
    }

    /// @notice Returns the price corresponding to the inputted id
    /// @param _id The id
    /// @return The price corresponding to this id
    function getPriceFromId(uint24 _id) external view override returns (uint256) {
        return BinHelper.getPriceFromId(_id);
    }
    ...
    function getSwapIn(
        ...
        while (true) {
            ...
            uint256 _price = BinHelper.getPriceFromId(_activeId);
            if (_reserve != 0) {
    ...
    function createLBPair(
        ...
        pair = factory.createLBPair(_tokenX, _tokenY, _activeId);
    }
    ...
    /// @return depositIds Bin ids where the liquidity was actually deposited
    /// @return liquidityMinted Amounts of LBToken minted for each bin
    function addLiquidityETH(LiquidityParameters calldata _liquidityParameters)
        external
        payable
        override
        returns (uint256[] memory depositIds, uint256[] memory liquidityMinted)
    {
        ...
        if (_liquidityParameters.tokenX == weth && _liquidityParameters.amountX == msg.value) {
            _wethDepositAndTransfer(address(_LBPair), msg.value);
            _liquidityParameters.tokenY.safeTransferFrom(msg.sender, address(_LBPair), _liquidityParameters.amountY);
        } else if (_liquidityParameters.tokenY == weth && _liquidityParameters.amountY == msg.value) {
            _liquidityParameters.tokenX.safeTransferFrom(msg.sender, address(_LBPair), _liquidityParameters.amountX);
            _wethDepositAndTransfer(address(_LBPair), msg.value);
        } else
            // revert LBRouter__WrongEthLiquidityParameters(
    ...
    function removeLiquidity(
        ...
        ILBPair _LBPair = _getLBPairInformation(_tokenX, _tokenY);
    ...
    /// @param _amountETHMin The min amount to receive of ETH
    /// @param _ids The list of ids to burn
    /// @param _amounts The list of amounts to burn of each id in `_ids`
    /// @param _to The address of the recipient
    /// @param _deadline The deadline of the tx
    ...
    function removeLiquidityETH(
        ...
        address payable _to,
        uint256 _deadline
    ) external override ensure(_deadline) returns (uint256 amountToken, uint256 amountETH) {
        ILBPair _LBPair = _getLBPairInformation(_token, IERC20(weth));

        bool _isETHTokenY = IERC20(weth) == _LBPair.tokenY();
        {
            if (!_isETHTokenY) {
                (_amountTokenMin, _amountETHMin) = (_amountETHMin, _amountTokenMin);
            }
            ...
            (amountToken, amountETH) = _isETHTokenY ? (_amountX, _amountY) : (_amountY, _amountX);
        }
    ...
    function swapExactTokensForTokens(
        ...
        address _to,
        uint256 _deadline
    ) external override ensure(_deadline) returns (uint256 amountOut) {
        address[] memory _pairs = _getPairs(_tokenPath);

        _tokenPath[0].safeTransferFrom(msg.sender, _pairs[0], _amountIn);

        amountOut = _swapExactTokensForTokens(_amountIn, _pairs,  _tokenPath, _to);
    ...
    /// @param _amountIn The amount of token to send
    /// @param _amountOutMinETH The min amount of ETH to receive
    /// @param _tokenPath The swap path using the binSteps following `_pairBinSteps`
    /// @param _to The address of the recipient
    /// @param _deadline The deadline of the tx
    /// @return amountOut Output amount of the swap
    function swapExactTokensForETH(
        uint256 _amountIn,
        uint256 _amountOutMinETH,
        IERC20[] memory _tokenPath,
        address payable _to,
        uint256 _deadline
    ) external override ensure(_deadline)  returns (uint256 amountOut) {
        // if (_tokenPath[_pairBinSteps.length] != IERC20(weth))
        ...
        amountOut = _swapExactTokensForTokens(_amountIn, _pairs, _tokenPath, address(this));

        if (_amountOutMinETH > amountOut) revert LBRouter__InsufficientAmountOut(_amountOutMinETH, amountOut);

        weth.withdraw(amountOut);
        _safeTransferETH(_to, amountOut);
    }
    ...
    function swapExactETHForTokens(
        ...
        if (_tokenPath[0] != IERC20(weth)) revert LBRouter__InvalidTokenPath(address(_tokenPath[0]));

        address[] memory _pairs = _getPairs( _tokenPath);

        _wethDepositAndTransfer(_pairs[0], msg.value);

        amountOut = _swapExactTokensForTokens(msg.value, _pairs,  _tokenPath, _to);
    ...
    function swapTokensForExactTokens(
        ...
        address _to,
        uint256 _deadline
    ) external override ensure(_deadline)  returns (uint256[] memory amountsIn) {
        address[] memory _pairs = _getPairs( _tokenPath);
        amountsIn = _getAmountsIn( _pairs, _tokenPath, _amountOut);
        ...
        uint256 _amountOutReal = _swapTokensForExactTokens(_pairs, _tokenPath,  _to);
    ...
    /// @param _amountETHOut The amount of ETH to receive
    /// @param _amountInMax The max amount of token to send
    /// @param _tokenPath The swap path using the binSteps following `_pairBinSteps`
    /// @param _to The address of the recipient
    /// @param _deadline The deadline of the tx
    ...
    function swapTokensForExactETH(
        ...
        address[] memory _pairs = _getPairs( _tokenPath);
        amountsIn = _getAmountsIn( _pairs, _tokenPath, _amountETHOut);

        if (amountsIn[0] > _amountInMax) revert LBRouter__MaxAmountInExceeded(_amountInMax, amountsIn[0]);

        _tokenPath[0].safeTransferFrom(msg.sender, _pairs[0], amountsIn[0]);

        uint256 _amountOutReal = _swapTokensForExactTokens(_pairs, _tokenPath,  address(this));

        if (_amountOutReal < _amountETHOut) revert LBRouter__InsufficientAmountOut(_amountETHOut, _amountOutReal);

        weth.withdraw(_amountOutReal);
        _safeTransferETH(_to, _amountOutReal);
    }

    /// @notice Swaps ETH for exact tokens while performing safety checks
    /// @dev Will refund any ETH amount sent in excess to `msg.sender`
        ...
        returns (uint256[] memory amountsIn)
    {
        ...
        _wethDepositAndTransfer(_pairs[0], amountsIn[0]);

        uint256 _amountOutReal = _swapTokensForExactTokens(_pairs,  _tokenPath,  _to);

        if (_amountOutReal < _amountOut) revert LBRouter__InsufficientAmountOut(_amountOut, _amountOutReal);

        if (msg.value > amountsIn[0]) _safeTransferETH(msg.sender, msg.value - amountsIn[0]);
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        _swapSupportingFeeOnTransferTokens(_pairs,  _tokenPath, _to);
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        uint256 _balanceBefore = weth.balanceOf(address(this));

        _tokenPath[0].safeTransferFrom(msg.sender, _pairs[0], _amountIn);

        _swapSupportingFeeOnTransferTokens(_pairs, _tokenPath, address(this));

        amountOut = weth.balanceOf(address(this)) - _balanceBefore;
        if (_amountOutMinETH > amountOut) revert LBRouter__InsufficientAmountOut(_amountOutMinETH, amountOut);
    ...
    function _getLBPairInformation(
        ...
        ILBPair _LBPair = factory.getLBPairInformation(_tokenX, _tokenY).LBPair;
        // if (address(_LBPair) == address(0))
        //     revert LBRouter__PairNotCreated(address(_tokenX), address(_tokenY));
        return _LBPair;
    }
    ...
    function _getPair(
        ...
        _pair = address(_getLBPairInformation(_tokenX, _tokenY));
    }

    function _getPairs( IERC20[] memory _tokenPath)
        private
        view
        returns (address[] memory pairs)
    {
        pairs = new address[](_tokenPath.length - 1);
        ...
        unchecked {
            for (uint256 i; i < pairs.length; ++i) {
                ...
                pairs[i] = _getPair( _token, _tokenNext);
            }
        }
    }
    ...
    /// @param _to The address of the recipient
    /// @param _amount The ETH amount to send
    function _safeTransferETH(address _to, uint256 _amount) private {
        (bool success, ) = _to.call{value: _amount}("");
        // if (!success) revert LBRouter__FailedToSendETH(_to, _amount);
    }

    /// @notice Helper function to deposit and transfer weth
    /// @param _to The address of the recipient
    /// @param _amount The ETH amount to wrap
