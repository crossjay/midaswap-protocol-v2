...
library TreeMath {
    ...
    //         (_binId, bit) = _getIdsFromAbove(_binId);
    //         current = _tree[2][_binId] >> bit;

    //         return current &= 1;
    //     }
    // }

    //return 最新的activebin和bestoffer
    /////////////////////////////////////////////
    /////////////////////////////////////////////
    function updateActiveBin(
        mapping(uint256 => uint256)[3] storage _tree,
        mapping(uint256 => uint256)[3] storage _tree2,
        IMidasPair721.PairInformation memory pair
    ) internal view returns (uint24 _bin , uint24 _bin2) {
        if (pair.reserveX != 0) {
            _bin2 = findFirstBin(_tree2 , 0 , false);
            _bin = findFirstBin(_tree , _bin2 + 1 , true);
        }else {
            _bin = findFirstBin(_tree , type(uint24).max , true);
        }
    }
